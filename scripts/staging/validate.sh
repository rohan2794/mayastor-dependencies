...
CHART_NAME="${CHART_NAME:-mayastor}"
...
if [[ -z "$TAG" ]]; then
    if [[ "$CHART_VALIDATE" == "true" ]]; then
        ...
        log_fatal "❌ Error: --tag is required when --chart=false (Chart.yaml not used)"
    fi
fi
...
# --- Check tag existence on remote for staging ---
...
# --- Validation execution (unchanged) ---
VERSION="${TAG#v}"
...
case "$TRIGGER" in
    staging)
        check_images
        [[ "$CHART_VALIDATE" == "true" ]] && check_chart || echo "Skipping chart validation (--chart=false)"
        ;;
    release)
        [[ "$CHART_VALIDATE" == "true" ]] && check_chart || echo "Skipping chart validation (--chart=false)"
        ;;
